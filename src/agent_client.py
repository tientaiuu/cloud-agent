...
class AgentCoreClient:
    ...
    def __init__(self, config_path: str = None, debug: bool = False, local_mode: bool = False):
        ...
        if not local_mode:
            ...
            self.agentcore_config = {
                'runtime': {
                    'p_agent': {'arn': 'local-p-agent', 'name': 'Local Production Agent'},
                    'q_agent': {'arn': 'local-q-agent', 'name': 'Local Test Agent'}
                },
                'agents': {'payload_formats': {'p_agent': 'direct', 'q_agent': 'direct'}},
    ...
    def _get_runtime_url(self, agent_type: str) -> str:
        ...
        if agent_type == "p_agent":
            runtime_arn = self.agentcore_config['runtime']['p_agent']['arn']
        elif agent_type == "q_agent":
            runtime_arn = self.agentcore_config['runtime']['q_agent']['arn']
        else:
    ...
    def display_available_runtimes(self) -> List[str]:
        ...
        # Check for Production agent
        if 'p_agent' in runtime_config:
            p_agent = runtime_config['p_agent']
            if p_agent.get('arn'):
                runtime_names.append('p_agent')
                print(f"1. Production Agent")
                print(f"   Name: {p_agent.get('name', 'N/A')}")
                if self.debug:
                    print(f"   ARN: {p_agent.get('arn', 'N/A')}")
        ...
        if 'q_agent' in runtime_config:
            q_agent = runtime_config['q_agent']
            if q_agent.get('arn'):
                runtime_names.append('q_agent')
                print(f"2. Test Agent")
                print(f"   Name: {q_agent.get('name', 'N/A')}")
                if self.debug:
                    print(f"   ARN: {q_agent.get('arn', 'N/A')}")
                print(f"   Status: ✅ Available")
            else:
                print(f"2. Test Agent")
                print(f"   Status: ❌ Not deployed")
